...
depth = 600
if Condition == 'SUB':
    for l in range(0,int(round(depth/(gcm/den),2))):
        reg_surface.append((-gcm/(den*100)*l+yModel[0]))
        den_correction.append(den)
    sub = len(reg_surface)
    yModel = reg_surface[::-1] +yModel 
...
while yFinal[counter+sub] <= regolith + 25000:
    ...
    for i in read:
        if 'FLAG1' in i:
            ...
            for j in range(3,len(xFinal)+3-counter):
                if j-1 < 9 + plus1:
                    if xFinal[j-3] == 1.7:
                        newfile.append(str(j-plus1) + '   2  ' + '-' + str(xFinal[j-3]) + '  ' + str(count) + '  ' + str(-(count+1)) + '\n')    
                    else:
                        newfile.append(str(j-plus1) + '   1  ' + '-' + str(xFinal[j-3]) + '  ' + str(count) + '  ' + str(-(count+1)) + '\n')    
                    count+=1
                else:
                    if xFinal[j-5] == 1.7:
                        newfile.append(str(j-plus1) + '  2  ' + '-' + str(xFinal[j-5]) + '  ' + str(count) + '  ' + str(-(count+1)) + '\n')    
                    else:
                        newfile.append(str(j-plus1) + '  1  ' + '-' + str(xFinal[j-5]) + '  ' + str(count) + '  ' + str(-(count+1)) + '\n')    
                    count+=1
...
for q in range(0,counter):
    ...
    ### Make the definition for the regions (reg)
            if 'FLAG7' in i:
                a = re.sub('FLAG7', 'reg = { ' + str(3-plus1) + ' - ' + str(len(xFinal)-q+3-plus1-1) + ' }' , str(i))
                newfile.append(a)
        #        k = np.linspace(3,endnum[q], endnum[q]-2)
        #        for i in range(0,len(k)):
        #            if i+1<10 and k[i]<10 + plus1:
        #                newfile.append('  ' + str(i+1) + '     ' + str(int(k[i])-plus1) + '\n')
        #            elif i+1<10  and k[i] >=10:
        #                newfile.append('  ' + str(i+1) + '    ' + str(int(k[i])-plus1) + '\n')
        #            else:
        #                newfile.append(' ' + str(i+1) + '    ' + str(int(k[i])-plus1) + '\n')
            else: 

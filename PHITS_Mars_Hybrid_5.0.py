...
while yFinal[counter+sub] <= regolith + 25000:
    File = open('PHITS_Template2.dat', 'r')
    read = File.readlines()
    newfile = []
    count = counter
    for i in read:
        if 'FLAG1' in i:
            ...
            if Condition == 'REG':
                ...
                newfile.append(str(9998-len(yFinal)+counter) + '        ' + 'so' + '            ' + str(yFinal[counter]-50) + '\n')
            ...
            if Condition == 'REG' or Condition == 'SUB':
                ...
                newfile.append('3   2  ' + str(-1.7) + '  ' + str((count)) +  '  ' + str(-(count+1)) + '\n')
            for j in range(3,len(xFinal)+3-counter-1):
                if j-1 < 9 + plus1-1:
                    if xFinal[j-3] == 1.7:
                        newfile.append(str(j-plus1+1) + '   2  ' + '-' + str(xFinal[j-2]) + '  ' + str(count+1) + '  ' + str(-(count+2)) + '\n')    
                    else:
                        newfile.append(str(j-plus1+1) + '   1  ' + '-' + str(xFinal[j-2]) + '  ' + str(count+1) + '  ' + str(-(count+2)) + '\n')    
                    count+=1
                else:
                    if xFinal[j-2] == 1.7:
                        newfile.append(str(j-plus1+1) + '  2  ' + '-' + str(xFinal[j-2]) + '  ' + str(count+1) + '  ' + str(-(count+2)) + '\n')    
                        ...
                        newfile.append(str(j-plus1+1) + '  1  ' + '-' + str(xFinal[j-2]) + '  ' + str(count+1) + '  ' + str(-(count+2)) + '\n')    
                    count+=1
            newfile.append(str(j+2-plus1) + '  0     ' + str((count+1)) + '  ' + str(-(count+2)) + '\n')
            newfile.append(str(j+3-plus1) + ' -1       ' + str((count+2)) + '\n')
            endnum.append(j)
        elif 'FLAG9' in i:
            ### Make Importance Card
            maximp=3
            newfile.append('  1     ' + str(maximp) + '\n')
            if Condition == 'REG' or Condition == 'SUB':
                newfile.append('  2     '  + str(maximp) + '\n')
                newfile.append('  3     '  + str(maximp) + '\n')
            for j in range(3,len(xFinal)+3-counter-1):
                if j-1 < 9 + plus1-1:
                    if xFinal[j-3] == 1.7:
                        newfile.append('  ' + str(j-plus1+1) + '     ' + str(maximp*np.exp(-j/len(xFinal))) + '\n')    
                    else:
                        newfile.append('  ' + str(j-plus1+1) + '     ' + str(maximp*np.exp(-j/len(xFinal)))+ '\n')    
                else:
                    if xFinal[j-2] == 1.7:
...
### Make the definition for the regions (reg)
        if 'FLAG7' in i:
            if int(math.floor(k+3-plus1+1))%2 ==0:
                a = re.sub('FLAG7', 'reg = ' + str(int(len(yFinal)*2-4)) , str(i))
            else:
                a = re.sub('FLAG7', 'reg = ' + str(int(len(yFinal)*2-4)) , str(i))
            newfile.append(a)
        elif 'FLAG8' in i:
            for p in range(3,int(len(yFinal)+1)):
                a = re.sub('FLAG8', '  ' + str(p) + '      ' + str(p+1) + '     (4*pi*' + str(yFinal[p-1]) + '**2)', str(i))
